...
public class FCommandEx implements CommandExecutor {
    ...
    public FCommandEx() {
        ...
        factions = Factions.factionsConfig;
    }
                ...
                if (factionList != null && factionList.size() != 0) {
                    for (Faction faction : factionList) {
                        if (faction == null)
                            continue;
                        if (faction.name.equalsIgnoreCase(args[1])) {
                            if (faction.invites.contains(pSender.getUniqueId().toString())) {
                                faction.invites.remove(pSender.getUniqueId().toString());
            ...
            case "invite":
                ...
                if (factionList != null && factionList.size() != 0) {
                    for (Faction faction : factionList) {
                        ...
                        faction.invites.add(Bukkit.getOfflinePlayer(args[1]).getUniqueId().toString());
